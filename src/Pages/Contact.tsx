const Contact = () => {
    return (
        <div className="flex-column justify-center bg opacity-50 h-2/3 w-3/4 mx-auto bg-black mt-10 rounded-lg">
            <h1 className="border-solid border-2 border-gray-400 text-center rounded-md mx-auto text-4xl font-mono">Contact:</h1>

            <h2 className="mx-auto mt-60 text-xl text-center">Feel free to contact me at <a className="underline" href="mailto:katlegobairvin@gmail.com">katlegobairvin@gmail.com</a></h2>
        </div>
    )
};

export default Contact